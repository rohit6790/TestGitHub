			...
			for (int colNum = 0; colNum < sheet.getColumnCount(); ++colNum) {
				...
				if (colName.trim().equalsIgnoreCase(executionMode)) {
					for (int rowNum = 1; rowNum < sheet.getRowCount(); ++rowNum) {
						String cellValue = sheet.getValueAt(colNum, rowNum)
								.toString();
						if (cellValue.trim().isEmpty()) {
							break;
						}
						if (Boolean.parseBoolean(cellValue)) {
							String testCase = sheet.getValueAt(1, rowNum)
									.toString();
							String browser = sheet.getValueAt(2, rowNum)
									.toString();
							...
							testCases.put(testCase, new ArrayList<String>(
									browsers));
						}
					}
					break;
		...
		for (String testCaseName : testCases.keySet()) {
			try {
				...
				for (int i = 1; i < sheet.getRowCount(); i++) {
					...
					actions.add(action);
					actionComponent.put(testCaseName, new ArrayList<String>(
							actions));
				}
			} catch (Exception e) {
				System.out.println("Ods file for the " + testCaseName
