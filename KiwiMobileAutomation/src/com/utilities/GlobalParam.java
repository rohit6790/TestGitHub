...
public abstract class GlobalParam {
	...
	private static void loadSuiteProperties() { // ##U
		...
		String fileLoc = GlobalParam.CURRENT_PROJECT_PATH + GlobalParam.slash + "suiteExecution.properties";
		try {
			File f = new File(fileLoc);
			FileInputStream fis = new FileInputStream(f);
			suiteProp.load(fis);
			...
			while (e.hasMoreElements()) {
				...
				if (prop.startsWith("cap_")) {
					...
					if (capName.equals("app"))
						capVal = System.getProperty("user.dir") + GlobalParam.slash + "app" + GlobalParam.slash
								+ capVal;
